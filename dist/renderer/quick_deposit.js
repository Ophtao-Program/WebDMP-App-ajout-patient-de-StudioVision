...
(() => {
    ...
    btnSend.addEventListener('click', async () => {
        ...
        if (!type) {
            renderStep('type', 'error', 'Choisissez un type de document.');
            typeSelect.focus();
            return;
        }
        if (!title) {
            renderStep('titre', 'error', 'Indiquez un titre.');
            titleInput.focus();
            return;
        }
        if (!current.existe) {
            renderStep('preflight', 'error', `Fichier introuvable : ${current.filePath}`);
            return;
        }
...
//# sourceMappingURL=quick_deposit.js.map
